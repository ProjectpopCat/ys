...
local PresetColor = Color3.fromRGB(255, 104, 5)
...
function lib:Window(text, preset, closebind)
    ...
    PresetColor = preset or Color3.fromRGB(255, 104, 5)
    fs = false
    ...
    Title.Text = text
    Title.TextColor3 = Color3.fromRGB(81, 81, 81)
        ...
        if io.KeyCode == CloseBind then
            if uitoggled == false then
                if Main and Main:IsA("GuiObject") then
                    Main:TweenSize(UDim2.new(0, 0, 0, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, 0.05, true)
                end
                uitoggled = true
                wait(0.05)
                ui.Enabled = false
...
function tabcontent:Slider(text, min, max, start, callback)
    ...
    local function updateSlider(input)
        ...
        local newValue = math.floor(pos * (max - min) + min)
